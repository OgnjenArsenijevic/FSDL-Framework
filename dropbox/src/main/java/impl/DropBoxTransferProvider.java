...
public class DropBoxTransferProvider extends AbstractDropBoxProvider implements TransferProvider {
    ...
    public boolean initStorage(String absolutePath) throws InitStorageException
    {
       ...
       /* int cnt = 0;
        ...
        if (doesFolderExist(ROOT_DIRECTORY_PATH))
        {
            adminExists = true;
            return true;
        }
    ...
    public void createFolder(int num, String destinationPath, String folderName, ArrayList<String> parameters) throws InvalidFormatException, UserException, PermissionException, FolderException
    {
        ...
        if (destinationPath.contains(".") || destinationPath.charAt(destinationPath.length() - 1) == '/')
        ...
        String fullPathAndName = destinationPath + "/" + folderName;
        DbxClientV2 dbxClient = getClient();
        try
        {
            dbxClient.files().getMetadata(destinationPath);
        }
        catch (GetMetadataErrorException e)
        {
            if (e.errorValue.isPath())
            {
                ...
                if (le.isNotFound())
                {
                    ...
                    try
                    {
                        dbxClient.files().createFolderV2(destinationPath);
                    }
        ...
        {
            throw new FolderException("Error while creating folder");
        }
        if(num == 0)
    ...
    public void uploadZip(int num, String zipName, ArrayList<String> userPCfilePaths, String destinationPath, ArrayList<String> parameters, HashMap<String,String> metadata) throws InvalidFormatException, NetworkErrorException, UserException, PermissionException, FolderException, FileException
    {
        ...
        if(zipName.contains("/") || zipName.contains("\\") || zipName.contains("."))
        ...
        for(int i=1;i<=num;i++)
        {
            ...
            try
            {
                ...
                {
                    if(userPCfilePath.charAt(j)=='/' || userPCfilePath.charAt(j)=='\\')
                        idx=j;
                }
                tmp+="/";
    ...
    private boolean upload(String localPath, String remotePath)
    {
       ...
       // System.out.println("Remote path: " + remotePath);
        ...
        {
            getClient().files().uploadBuilder(remotePath).withMode(WriteMode.OVERWRITE).uploadAndFinish(in);
            return true;
        }
    ...
    private void uploadWithOption(String localPath, String remotePath, ArrayList<String> parameters, HashMap<String,String> metadata) throws NetworkErrorException, FileException
    {
        ...
        try
        {
            ...
            getClient().files().getMetadata(remotePath);
            ...
            try (InputStream in1 = new FileInputStream(localPath))
            {
                getClient().files().uploadBuilder(remotePath).withMode(WriteMode.OVERWRITE).uploadAndFinish(in1);
                in.close();
                if(!parameters.contains("-m"))
                    return;
                uploadMetadata(newRemotePath,metadata);
            }
            catch (IOException | DbxException ee)
            {
                in.close();
    ...
    public void downloadZip(String remotePath, String localPath) throws InvalidFormatException,  UserException, PermissionException
    {
        ...
        if(!doesFolderExist(getAbsolutePathForPath(remotePath)))
            return;

        remotePath = getAbsolutePathForPath(remotePath);
        try (OutputStream downloadFile = new FileOutputStream(localPath)) {
            getClient().files().downloadZipBuilder(remotePath).download(downloadFile);
        } catch (DbxException | IOException e) {
            //e.printStackTrace();
        }
    }
    ...
    @Override
    public List<String> storageContent(String remotePath) {
        /*List<FileMetadata> results = directoryContent(ROOT_DIRECTORY_PATH);
        return results;*/
        try
        {
            List<String> listOfItems = new ArrayList<>();
            listEverything(listOfItems,remotePath);
            return listOfItems;
        }
        catch (DbxException e)
        {
    ...
    private void listEverything(List<String> lista, String remotePath) throws DbxException
    {
        Metadata data = getClient().files().getMetadata(remotePath);
        if (data instanceof FolderMetadata){
            lista.add(remotePath);
            ListFolderResult listFolderResult = getClient().files().listFolder(remotePath);
            List<Metadata> listaData = listFolderResult.getEntries();
            for (Metadata metadata : listaData)
                listEverything(lista,metadata.getPathLower());
        }else {
            lista.add(remotePath);
        }
    }

    private boolean doesFolderExist(String absolutePath)
    {
        try
        {
            getClient().files().getMetadata(absolutePath);
            return true;
        }
        catch (Exception e)
        {
    ...
    private boolean create(String fullPathAndName, ArrayList<String> parameters)
    {
        ...
        String split[] = fullPathAndName.split("/");
        try
        {
            CreateFolderResult folder = getClient().files().createFolderV2(fullPathAndName);
            return true;
        }
    ...
    private boolean delete(String remotePath)
    {
        try
        {
            DeleteResult metadata = getClient().files().deleteV2(remotePath);
